...
module.exports.run = async(client, message, args, embed) => {
    if (!message.member.roles.cache.has(ayar.roles.registerStaff) && !message.member.hasPermission(8)) return message.channel.send(embed.setDescription(`${admin}, Bu komutu kullanmak için yeterli yetkiye sahip değilsin.`)).sil(7)
    ...
    if (db.get(`taglıalım.${message.guild.id}`)) {
        if (!member.user.username.includes(ayar.guild.tag) && !member.roles.cache.has(ayar.roles.boosterRole) && !member.roles.cache.has(ayar.roles.boosterRole)) {
